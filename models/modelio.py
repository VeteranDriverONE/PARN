...
class LoadableModel(nn.Module):
    ...
    def load(model, path, device):
        ...
        """
        cpk = torch.load(path, map_location=torch.device(device))
        model.unet_model.load_state_dict(cpk['unet_model'])
        for i,submodel in enumerate(model.cascade_networks.stems):
            submodel.load_state_dict(cpk[f"cascade {i}"])
        return model
